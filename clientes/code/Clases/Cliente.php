...
class Cliente
{
    ...
    function guardar()
    {
        $sql = "insert into clientes_datos_generales (nombreCliente, idTipoDocumentoIdentidad, numeroDocumento,
                      iva, idActividadEconomica, correo, telefono, 
                      idDepartamento,idMunicipio, direccion, idUsuarioRegistra) 
                values (:nombreCliente, :idTipoDocumentoIdentidad, :numeroDocumento, 
                        :iva, :idActividadEconomica,:correo, :telefono,
                        :idDepartamento, :idMunicipio,:direccion,:idUsuarioRegistra)";
    ...
    function actualizar()
    {
        $respuesta = new stdClass();
            ...
            $sql = "update clientes_datos_generales set nombreCliente =:nombreCliente, idTipoDocumentoIdentidad=:idTipoDocumentoIdentidad,
    ...
    {
        $respuesta = new stdClass();
        $sql = " update clientes_datos_generales set eliminado='S', idUsuarioElimina = {$this->idUsuario},
    ...
    {
        $sql = "select id, nombreCliente, idTipoDocumentoIdentidad, numeroDocumento,idActividadEconomica,
       iva, idDepartamento,idMunicipio, correo, telefono, direccion
       from clientes_datos_generales where id = {$this->id}";
        $query = $this->conexion->query($sql);
        if ($query->rowCount() > 0) {
    ...
    function obtenerClientesByTable()
    {
        $respuesta = new stdClass();
        $sql = "select cli.id, nombreCliente, telefono, correo,  act.nombreActividad,tdoc.nombre as tipoDocumento,
       numeroDocumento
    from clientes_datos_generales cli
